...
from gpflow.kernels import RBF, SeparateIndependent
...
def build_SVGPExpert(
    X: InputData,
    lengthscales: List[List[float]] = [[1.0, 1.0], [1.0, 1.0]],
    kernel_variance: List[float] = [1.0, 1.0],
    noise_variance: List[float] = [[1.0, 1.0], [1.0, 1.0]],
) -> SVGPExpert:
    output_dim = 2
    likelihood = Gaussian(variance=noise_variance)
    mean_function = Constant(c=[0.0] * output_dim)
    kernel_list = []
    for dim in range(output_dim):
        kernel_list.append(
            RBF(lengthscales=lengthscales[dim], variance=kernel_variance[dim])
        )
    kernel = SeparateIndependent(kernels=kernel_list)
