...
def generate_point_mass_dataset(
    ...
    """Generate and save transition dataset of VelocityControlledPointMass2DEnv
    ...
    save_dataset_filename = os.path.join(
        env_dir, "data/point_mass_" + str(num_states) + "_" + str(num_actions) + ".npz"
    )  # save dataset here
    if not overwrite:
...
def generate_random_point_mass_dataset(
    ...
    gating_bitmap_file = os.path.join(env_dir, "gating_mask.bmp")
    save_dataset_filename = os.path.join(
        env_dir, "data/point_mass_random_" + str(num_samples) + ".npz"
    )  # save dataset here
    if not overwrite:
        if os.path.exists(save_dataset_filename):
            print("File exists and no overwrite selected.")
...
if __name__ == "__main__":
    # generate data sets for each VelocityControlledPointMass2DEnv scenario (sub dirs)
    this_dir = os.getcwd()
    for path in os.scandir(this_dir):
        if path.is_dir():
            if "__pycache__" not in path.path:
                if "scenario-4" in path.path:
