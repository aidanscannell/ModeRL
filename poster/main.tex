...
\begin{minipage}{\textwidth}
  ...
  \begin{tikzpicture}[inner sep=0,outer sep=0]
     ...
     \node at (0,100) {
        \includegraphics[width=0.28\textwidth, trim= 0 30 326.5 5, clip]{../experiments/figures/greedy_and_myopic_comparisons.pdf}
        \includegraphics[width=0.65\textwidth, trim= 0 30 49 5, clip]{../experiments/figures/moderl_four_iterations_in_row.pdf}
...
% \alert{\bf Figure~1:}
  ...
  Right hand plots show four iterations of \alert{ModeRL}, which successfully navigates to the target with ``some level'' of constraint satisfaction during training.
  The $\delta\text{-mode-constrained}$ region (\tikz[baseline=-0.5ex]\draw [line width=1.25mm, black] (0,0) -- (1,0);) expands at each episode $i$.}
  \label{fig-main}
  \end{figure}
...
\begin{column}{\colwidth}
    ...
    %     every node/append .style={scale=1.3},
    %     every node/append .style={font=\normalsize},
    %     every latent/append .style={font=\normalsize}
    %     % fontscale/.style = {font=\relsize{#1}}
    %     ]
    %         \node[const] (x) {$(\state_{t}, \action_{t})$};
    %         \node[latent, left=of x, yshift=-1.4cm] (f) {$\mode{\latentFunc}(\state_{t}, \action_{t})$};
...
\begin{column}{\colwidth}
  ...
  \begin{figure}[!h]
    \begin{tikzpicture}[inner sep=0,outer sep=0]
      \node[rounded corners=6pt] at (0,200) {
     \includegraphics[width=0.5\columnwidth]{../experiments/figures/num_constrint_violations_constraint_levels_ablation.pdf}
          \includegraphics[width=0.5\columnwidth]{../experiments/figures/episode_return_constraint_levels_ablation.pdf}
      };
    \end{tikzpicture}
