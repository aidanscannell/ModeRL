...
def apply_mask_to_states(states, env, omit_data_mask=None):
    ...
    if omit_data_mask is None:
        ...
        raise ("omit_data_mask must be np.ndarray or filepath string for bitmap")
...
def transition_dynamics(state_action, env, num_states):
    ...
    state = state_action[:, :num_states]
    action = state_action[:, num_states:]
    delta_state = env.transition_dynamics(state, action)
    return delta_state.reshape(-1)


def generate_transitions_dataset(num_states, num_actions, env, omit_data_mask=None):
    ...
    num_actions = num_states.shape[0]
...
def generate_random_transitions_dataset(num_samples, env, omit_data_mask=None):
    """Generate dataset of state transitions

    :param num_states: number of samples to generate
    :param env: an instance of VelocityControlledQuadcopter2DEnv
    :param omit_data_mask: bitmaap used to omit data from dataset
    :returns: state_action_inputs [num_states*num_actions, state_dim+action_dim]
              delta_state_outputs [num_states*num_actions, state_dim]
    ...
    # action_dim = env.action_spec().shape[1]
    # action_dim = env.action_spec().shape[0]
    if len(env.action_spec().shape) == 2:
        action_dim = env.action_spec().shape[1]
    elif len(env.action_spec().shape) == 1:
        action_dim = env.action_spec().shape[0]
